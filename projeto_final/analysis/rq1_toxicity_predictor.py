"""Builds and evaluates a predictive model for next-minute toxicity.
Compares a baseline autoregressive model against an expanded model including on-field metrics (xT and Momentum).
...
def build_features(panel: pd.DataFrame) -> tuple[pd.DataFrame, list[str], list[str]]:
    ...
    # Target: toxicity_ratio at t+1
    df["target"] = df.groupby("game_id")["toxicity_ratio"].shift(-1)
    
    # Lag features (current and previous toxicity)
    df["toxicity_lag0"] = df["toxicity_ratio"]
    df["toxicity_lag1"] = df.groupby("game_id")["toxicity_ratio"].shift(1)
    ...
    baseline_cols = ["toxicity_lag0", "toxicity_lag1"]
    field_cols = ["xt_diff_t", "xt_home_t", "xt_away_t", "momentum_t", "momentum_diff_t"]
...
def main():
    ...
    metrics_df.to_csv(OUT_DIR / "tables" / "rq1_predictor_metrics.csv", index=False)
    
    # Fit final model to extract feature importances
    coef_df = train_and_get_coefs(df_clean, expanded_cols)
    coef_df.to_csv(OUT_DIR / "tables" / "rq1_predictor_coefficients.csv", index=False)
    print("\n=== Standardized Coefficients of the Expanded Model ===")
    print(coef_df)
    
    # Plot feature importances
    plt.figure(figsize=(8, 5))
    colors = ['#2e4053' if 'lag' in f else '#a93226' for f in coef_df["Feature"]]
    plt.barh(coef_df["Feature"], coef_df["Coefficient"], color=colors)
    plt.axvline(0, color="black", linewidth=0.8)
    plt.xlabel("Coeficiente Padronizado (Importância)")
    plt.title("Importância das Features na Predição de Toxicidade (t+1)")
    plt.gca().invert_yaxis()
    plt.tight_layout()
    plt.savefig(OUT_DIR / "figures" / "rq1_predictor_feature_importance.png", dpi=150)
    plt.close()
